...
class LastFmSettingsActivity : SettingsPageActivity() {
    ...
    override fun fillItems(items: ArrayList<UItem>, adapter: UniversalAdapter) {
        ...
        val currentUsername = LastFmStorage.username
        items.add(
            UItem.asButton(
                BUTTON_USERNAME,
        ...
        items.add(
            mkTwoLineCheckItem(
                ...
                LastFmStorage.useOwnApiKey
            )
        )

        if (LastFmStorage.useOwnApiKey) {
            val key = LastFmStorage.personalApiKey
            items.add(
                UItem.asButton(
                    BUTTON_PERSONAL_KEY,
    ...
    override fun onClick(item: UItem, view: View, position: Int, x: Float, y: Float) {
        when (item.id) {
            ...
            TOGGLE_SHOW_ON_PROFILE -> {
                val new = !LastFmStorage.showOnProfile
                LastFmStorage.showOnProfile = new
                (view as? NotificationsCheckCell)?.isChecked = new
                (view as? TextCheckCell)?.isChecked = new
                LastFmBioSyncHelper.onShowOnProfileToggled()
            }

            TOGGLE_USE_OWN_KEY -> {
                val new = !LastFmStorage.useOwnApiKey
                LastFmStorage.useOwnApiKey = new
    ...
    private fun showUsernameDialog() {
        ...
        val input = EditText(ctx).apply {
            ...
            setText(LastFmStorage.username)
        ...
        AlertDialog.Builder(ctx)
            ...
            .setPositiveButton(LocaleController.getString(R.string.OK)) { _, _ ->
                ...
                if (newName != LastFmStorage.username) {
                    LastFmStorage.username = newName
                    LastFmBioSyncHelper.onUsernameChanged()
                    listView.adapter.update(true)
                }
            }
    ...
    private fun showPersonalKeyDialog() {
        ...
        val input = EditText(ctx).apply {
            ...
            setText(LastFmStorage.personalApiKey)
            inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS
            isSingleLine = true
            textSize = 16f
        }
        ...
        AlertDialog.Builder(ctx)
            ...
            .setPositiveButton(LocaleController.getString(R.string.OK)) { _, _ ->
                ...
                LastFmStorage.personalApiKey = newKey
                listView.adapter.update(true)
            }
            .setNegativeButton(LocaleController.getString(R.string.Cancel), null)
            .show()
